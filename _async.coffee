# This file is an add-on to the FormValidator that replaces some methods for supporting asynchronous ajax requests being used in validators.
...
_validate_element: (elem, data, value_info, options) ->
    ...
    sequence = new @constructor._sequence_class([
        #########################################################
        # PHASE 1: validate dependencies (no matter if the value has changed because dependencies could have changed)
        {
            func: () ->
                return @_validate_dependencies(elem, data, options)
            scope: @
        }
        {
            func: (dependency_errors, dependency_elements, dependency_mode, valid_dependencies) ->
            ...
            func: () ->
                if prev_phases_valid or not options.stop_on_error
                    ...
                    if value_has_changed or (data.dependency_changed and data.valid_dependencies and data.errors[phase].length is 0)
                        return @_validate_value(elem, data, value_info)
        ...
        # PHASE 3 - validate constraints
        {
            func: () ->
                if value_has_changed
                    if prev_phases_valid or not options.stop_on_error
                        return @_validate_constraints(elem, data, value)
                else
                    if prev_phases_valid or not options.stop_on_error
