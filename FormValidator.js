...
(function() {
  var BUILD_MODES, CACHED_FIELD_DATA, DEBUG, ERROR_MESSAGE_CONFIG, ERROR_MODES, ERROR_OUTPUT_MODES, VALIDATION_PHASES, VALIDATION_PHASES_SINGULAR, build_mode_helpers, constraint_validator_groups, constraint_validator_options, constraint_validators, default_message_builder, error_message_builders, get_combined_key, get_permutations, locale_build_mode_helpers, locales, part_evaluator, permutation_cache, validators,
  ...
  get_permutations = function(input) {
    var permute, results;
    permute = function(arr, results, memo) {
      var cur, i, j, ref, results1;
      if (memo == null) {
        memo = [];
      }
      results1 = [];
  ...
  get_combined_key = function(keys, locale, key_prefix, key_suffix) {
    ...
    while (keys.length > 0) {
      ...
      if (permutation_cache[key] != null) {
        return key;
      }
      ref = get_permutations(keys);
  ...
  error_message_builders[VALIDATION_PHASES.CONSTRAINTS] = function(errors, phase, build_mode, locale) {
    var error, group, grouped_errors, idx, j, k, key, key_prefix, keys, l, len, len1, len2, parts;
    parts = [];
    grouped_errors = [];
    for (j = 0, len = constraint_validator_groups.length; j < len; j++) {
      group = constraint_validator_groups[j];
    ...
    for (l = 0, len2 = grouped_errors.length; l < len2; l++) {
      errors = grouped_errors[l];
      keys = (function() {
        var len3, m, results1;
        results1 = [];
    ...
    return default_message_builder((VALIDATION_PHASES_SINGULAR[phase].toLowerCase()) + "_" + (build_mode.toLowerCase()), build_mode, locale, parts);
  };
