...
var $_from_arr, BUILD_MODES, CONSTRAINT_VALIDATOR_OPTIONS, DEBUG, DEFAULT_ATTR_VALUES, DEPENDENCY_CHANGE_ACTIONS, DEPENDENCY_CHANGE_ACTION_DURATION, ERROR_MESSAGE_CONFIG, ERROR_MODES, ERROR_OUTPUT_MODES, EXPOSED_CONSTANTS, Maker, MessageBuilder, MessageDataCreator, OPTIONAL_CACHE, OrderedDict, PROGRESS_MODES, REQUIRED_CACHE, VALIDATION_PHASES, VALIDATION_PHASES_SINGULAR, _permutation_cache, constraint_validator_groups, constraint_validator_options_in_locale_key, constraint_validators, get_combined_key, get_permutations, get_subsets, group_arr_by, include_constraint_option_in_locale_key, locale_message_builders, locales, message_builder_helper, message_builders, message_data_creators, parse_deps, part_evaluator, set, set_add, set_remove, toposort, validators,
...
window.FormValidator = (function() {
  ...
  if (typeof Sequence !== "undefined") {
    FormValidator._sequence_class = Sequence;
  } else {
    FormValidator._sequence_class = null;
  }
  ...
  FormValidator.set_sequence_class = function(sequence_class) {
    this._sequence_class = sequence_class;
    return this;
  };
  ...
  FormValidator._get_error_message_for_element = function(element, errors, build_mode, locale, delimiter) {
    ...
    ref = ERROR_MESSAGE_CONFIG.PHASE_ORDER;
    for (l = 0, len = ref.length; l < len; l++) {
      phase = ref[l];
  ...
  *
  ...
  function FormValidator(form, form_modifier, options) {
    ...
    this.dependency_change_action = options.dependency_change_action || this.form.attr("data-fv-dependency-change-action") || DEPENDENCY_CHANGE_ACTIONS.DEFAULT;
    ...
    this.dependency_action_target_getter = options.dependency_action_target_getter || null;
  ...
  FormValidator.prototype._get_value = function(element, data) {
    var method, type, usedValFunc, value;
    type = data.type;
    usedValFunc = true;
    if (type === "checkbox" || type === "radio") {
      value = element.prop("checked") === true ? "checked" : "unchecked";
      method = "checkbox";
    } else if (type === "select") {
  ...
  FormValidator.prototype._set_value = function(element, data, method, value) {
    if (method === "checkbox") {
      element.prop("checked", value === "checked" ? true : false);
    } else if (method === "val") {
      element.val(value);
    } else {
      element.text(value);
  ...
  FormValidator.prototype._get_value_info = function(element, data) {
    var method, preprocess, ref, type, value, value_has_changed;
    type = data.type, preprocess = data.preprocess;
    ref = this._get_value(element, data), method = ref.method, value = ref.value;
    value_has_changed = value !== data.value;
    data.value = value;
  ...
  FormValidator.prototype._get_error_targets = function(element, type) {
    var target_list;
    target_list = (typeof this.error_target_getter === "function" ? this.error_target_getter(element, type) : void 0) || element.attr("data-fv-error-targets") || element.closest("[data-fv-error-targets]").attr("data-fv-error-targets") || DEFAULT_ATTR_VALUES.ERROR_TARGETS;
  ...
  FormValidator.prototype._group = function(fields) {
    ...
    for (i = l = 0, ref = fields.length; 0 <= ref ? l < ref : l > ref; i = 0 <= ref ? ++l : --l) {
      ...
      if (data.group == null) {
        data.group = elem.attr("data-fv-group") || elem.attr("name") || "";
    ...
    for (i = l = 0, ref = fields.length; 0 <= ref ? l < ref : l > ref; i = 0 <= ref ? ++l : --l) {
      ...
      if (elem_errors.length > 0) {
        if (options.messages === true) {
          message = CLASS._get_error_message_for_element(elem, elem_errors, this.build_mode, this.locale);
        } else {
          message = "";
        }
        result.push({
  ...
  FormValidator.prototype._validate_element = function(elem, data, value_info, options) {
    var constraint_name, dependency_elements, dependency_error, dependency_errors, dependency_mode, error, errors, is_required, l, len, len1, m, method, phase, prev_phases_valid, ref, ref1, ref2, result, temp, type, valid_dependencies, validation_res, value, value_has_changed;
    ...
    type = data.type;
    value = value_info.value, value_has_changed = value_info.value_has_changed, method = value_info.method;
    ...
    if (data.valid_dependencies && data.valid_value && data.valid_constraints) {
      ...
      if (data.postprocess === true || data.output_preprocessed === true) {
        ...
        this._set_value(elem, data, method, value);
  ...
  *
  ...
  FormValidator.prototype.validate = function(options) {
    var CLASS, data, default_options, elem, elem_errors, errors, fields, grouped_errors, is_required, l, len, ref, type, usedValFunc, value_info;
    if (options == null) {
      options = {};
    }
    default_options = {
    ...
    for (l = 0, len = fields.length; l < len; l++) {
      ...
      if (options.all === false && !is_required && (value_info.value.length === 0 || type === "radio" || type === "checkbox")) {
  ...
  FormValidator.prototype.get_progress = function(options) {
    var data, elem, errors, fields, group, groups, i, j, l, len, len1, len2, m, mode, n, num_optional, num_required, num_valid_optional, num_valid_required, o, r, result, sum;
    if (options == null) {
      options = {
        mode: PROGRESS_MODES.DEFAULT,
        recache: false
      };
    }
    if ((this.fields == null) || options.recache === true) {
    ...
    for (i = l = 0, len = groups.length; l < len; i = ++l) {
      ...
      for (j = m = 0, len1 = group.length; m < len1; j = ++m) {
        elem = group[j];
        if (!(elem instanceof jQuery)) {
          elem = (typeof group.eq === "function" ? group.eq(j) : void 0) || $(elem);
        }
        data = this._get_element_data(elem);
      ...
      if (num_required > 0) {
        if (mode === PROGRESS_MODES.PERCENTAGE) {
          result.push(num_valid_required / num_required);
        } else if (mode === PROGRESS_MODES.ABSOLUTE) {
          result.push({
